...
all_draw_data <- read_csv2("simdata/LawWatkinson_PosteriorDraws_PriorGradient.csv")
...
jpeg("./figs/SIFigLawWatkinsonVarFitEvals.jpeg",
...
jpeg("./figs/SIFigLawWatkinsonLambdaRatios.jpeg",
...
jpeg("./figs/SIFigLawWatkinsonBetaRatios.jpeg",
...
jpeg("./figs/SIFigLawWatkinsonVarianceTrends.jpeg",
...
alpha_diff_draws <- alpha_merge_draws %>%
  ...
  mutate(LambdaRatio = paste("Ratio =", LambdaRatio))

plVarAlphaDiffs <- ggplot(alpha_diff_draws, aes(x = value, y = InteractionID,
                                                fill = after_stat(x < 1))) +
  stat_halfeye() +
  facet_wrap(variable~LambdaRatio, nrow = 2, scales = "free") +
...
jpeg("./figs/SIFigLawWatkinsonVarAlphaDiffs.jpeg",
     width = 6000, height = 3500, res = 300)
plVarAlphaDiffs
dev.off()
...
for(cur_foc in Foc) {
  ...
  for(cur_ind in c("non", "foc")) {
    ...
    for(lambda_ratio in unique(all_draw_data$LambdaRatio)) {
      ...
      cur_car_cap_val <- ((cur_eta - 1) / cur_germ) ^ (1 / cur_draws[,2])
...
plCarryingCapacity <- ggplot(car_cap_plotting,
                             aes(x = log(Nbar),
                                 linetype = Induction,
                                 color = as.factor(LambdaRatio))) +
  geom_density() + xlim(c(0.001, 50)) +
  facet_wrap(~Focal, nrow = 1, scales = "free") +
  labs(x = "Logged Carrying Capacity", y = "") +
...
jpeg("./figs/SIFigLawWatkinsonVarCarryingCapacities.jpeg",
  ...
  for(cur_res in res_indices) {
    ...
    for(cur_ind_type in c("non", "foc", "res")) {
      ...
      for(lambda_ratio in unique(all_draw_data$LambdaRatio)) {
        ...
        cur_igrs <- cur_foc_germ * cur_draws[,1] / (1 + cur_res_germ * (cur_car_cap$Nbar ^ cur_draws[,2])) + (1 - cur_foc_germ) * cur_foc_surv
        #cur_igrs <- cur_draws[,1] / (1 + cur_car_cap$Nbar ^ cur_draws[,2])
        names(cur_igrs) <- "IGR"
...
jpeg("./figs/SIFigLawWatkinsonVarIGR.jpeg",
     width = 3500, height = 2500, res = 300)
plIGR
dev.off()
...
jpeg("./figs/SIFigLawWatkinsonMatrices.jpeg",
...
full_pred_data <- full_pred_data %>%
  mutate(PredSeeds = (Lambda / (1 + Beta + Resident.Density^Alpha)))
...
jpeg("./figs/SIFigLawWatkinsonFits.jpeg",
...
for(cur_foc in Foc) {
  ...
  for(cur_res in res_indices) {
    ...
    for(cur_ind_type in c("non", "res")) {
      ...
      cur_comp_ab_f <- log(cur_lambda_f - 1) / sqrt(cur_alpha_ff * cur_alpha_fr)
      cur_comp_ab_r <- log(cur_lambda_r - 1) / sqrt(cur_alpha_rr * cur_alpha_rf)
...
#jpeg("./figs/SIFigLawWatkinsonNFDiffs.jpeg",
...
plIndPatterns <- ggplot(melt_ind_data, aes(x = Focal, y = value, fill = after_stat(y < 1))) +
...
melt_ind_int_data <- ind_int_data %>%
  group_by(Focal, SpID) %>%
  summarise(MedianSusc = median(Susc), MedianStr = median(Str),
...
plIndSuscStr <- ggplot(melt_ind_int_data, aes(x = MedianSusc, y = MedianStr)) +
  ...
  #geom_smooth(formula = y ~ x, method = "lm", alpha = 0.25) +
  ...
  ggtitle("B") +
  labs(x = expression(atop("Induction Response Per Interaction", (log(alpha[ij]~"|"[j] / alpha[ij]~"|"[0])))),
       y = expression(atop("Induction Effect Per Interaction", (log(alpha[ji]~"|"[i] / alpha[ji]~"|"[0])))))
plIndSuscStr

jpeg("./figs/SIFigLawWatkinsonIndPatterns.jpeg",
     width = 4500, height = 1700, res = 300)
