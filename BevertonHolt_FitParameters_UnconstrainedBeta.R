...
num_iter <- 10000
in_lambda_ratio <- 0.1
in_ind_ratios <- c(0.1, 0.25, 0.5, 0.75, 1)

draw_data <- data.frame()

start_time <- Sys.time()

for(in_ind_ratio in in_ind_ratios) {
  
  print(paste0("Beta ratio ",
               which(in_ind_ratio == in_ind_ratios),
               " out of ",
               length(in_ind_ratios),
               "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"))
...
filename <- "BevertonHolt_PosteriorDraws_BetaGradient"
